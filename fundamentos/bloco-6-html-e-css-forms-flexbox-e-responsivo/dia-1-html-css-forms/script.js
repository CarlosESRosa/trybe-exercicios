// Não consegui resolver os exercícios desse dia, então decidi só copiar o gabarito para conseguir dar continuidade nos conteudos
...
function dateValidation(input, name) {
  ...
  let regex = /^\d\d\/\d\d\/\d\d\d\d$/; // regular expression

  if (!regex.test(input.value)) {
    // testa se nosso input está de acordo com o regex
    return {
      message: "Data: Formato inválido",
    };
  }
